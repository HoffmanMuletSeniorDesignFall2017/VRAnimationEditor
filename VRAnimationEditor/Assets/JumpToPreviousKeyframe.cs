...
public class JumpToPreviousKeyframe : MonoBehaviour
{
    ...
    public void OnClick()
    {
        ...
        {
            if (closestTime <= 0.01f || closestTime >= 0.99f)
            {
                animVis.keyframeWorkArea.GetComponent<KeyframeWorkArea>().timelineVisualizer.ChangeTime(0.01f);
            }
            else
            {
